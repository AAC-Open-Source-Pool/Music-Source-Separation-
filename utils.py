...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    transform = transforms.Compose(transform_list)
    # dataset = CustomTensorDataset('/home/moshe/ext/data/datasets/lin_specs/musdb_yedid/train/')
...
def write_2images(image_outputs, display_image_num, image_directory, postfix):
    ...
    __write_images(image_outputs, display_image_num, '%s/gen_b2a_%s.jpg' % (image_directory, postfix))
...
def prepare_sub_folder(output_directory):
    ...
    tests_directory = os.path.join(output_directory, 'tests')
    if not os.path.exists(tests_directory):
        print("Creating directory: {}".format(tests_directory))
        os.makedirs(tests_directory)
    return checkpoint_directory, image_directory, tests_directory
