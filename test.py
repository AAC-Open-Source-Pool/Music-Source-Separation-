...
class GenerateMusic:
    ...
    def stft_music(self):
        ...
        num_frames = stft_mag.shape[0] // 256
        if num_frames == 0:
            # handle short input case, e.g., pad or skip processing
            raise ValueError("Input too short to process: STFT frames < 256")
        stft_images_t = np.reshape(stft_mag[:num_frames*256, :], (num_frames, 256, stft_mag.shape[1]))
