...
import argparse
MIN_LEN = 65792  # minimum length for 256 frames at n_fft=512, hop_length=256
...
def create_songs(config, checkpoint, output_folder='./outputs',
    ...
    enhance = 9
    MIN_LEN = 512  # Minimum length to avoid STFT errors (n_fft size)

    for musicfile_mix in tqdm(music_mix_list):
        filepath = os.path.join(input_folder, musicfile_mix)
        sample_rate = 20480
        music_array, old_sr = sf.read(filepath)

        # Resample to sample_rate (20480 Hz)
        if len(music_array.shape) > 1:
            music_array = librosa.resample(y=music_array.T, orig_sr=old_sr, target_sr=sample_rate).T
        else:
            music_array = librosa.resample(y=music_array, orig_sr=old_sr, target_sr=sample_rate)

        if len(music_array.shape) > 1:
            # Stereo
            channels, length = music_array.shape
            if length < MIN_LEN:
                pad_len = MIN_LEN - length
                music_array = np.pad(music_array, ((0,0), (0, pad_len)), mode='constant')
        else:
            # Mono
            length = music_array.shape[0]
            if length < MIN_LEN:
                pad_len = MIN_LEN - length
                music_array = np.pad(music_array, (0, pad_len), mode='constant')
        ...
        recon_vocals = librosa.resample(y=recon_vocals, orig_sr=sample_rate, target_sr=old_sr)
        recon_inter = librosa.resample(y=recon_inter, orig_sr=sample_rate, target_sr=old_sr)

        # Save outputs
        sf.write(os.path.join(output_folder, 'vocals_' + musicfile_mix), recon_vocals.transpose(), old_sr)
        sf.write(os.path.join(output_folder, 'accompaniment_' + musicfile_mix), recon_inter.transpose(), old_sr)
